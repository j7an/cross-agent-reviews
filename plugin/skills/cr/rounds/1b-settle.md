...
`uv run python scripts/cr_state_read.py --slug <slug> --artifact-type <type>` (use the local file). The 1a findings are at `.cross-agent-reviews/<slug>/<artifact_type>/round-1a.json` under `agents[].findings`.
