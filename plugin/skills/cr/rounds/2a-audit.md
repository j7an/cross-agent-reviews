...
If the cross-artifact slice is present and the artifact is the plan, re-run `uv run python scripts/cr_extract_placeholders.py --spec-path <SPEC> --plan-path <PLAN>` against the corrected plan. The agent_id 6 sub-agent receives the new extractor report.
