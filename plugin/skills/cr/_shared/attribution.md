...
In v0.1.x, the six top-level skills (`cr-1a-audit` … `cr-3b-final-settle`) were
consolidated into a single state-driven `cr` skill. The dispatch and review
patterns inherited from Superpowers (`dispatching-parallel-agents`,
`subagent-driven-development`) remain unchanged; only the skill packaging changed.
