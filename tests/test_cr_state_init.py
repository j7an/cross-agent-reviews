...
def run(args, cwd, stdin=None, env=None):
    ...
    Inherits `os.environ` so coverage.py's subprocess startup config reaches
    the subprocess. Setting PYTHONPATH=REPO_ROOT lets `sitecustomize.py` (at
    the repo root) load so `coverage.process_startup()` runs in each
    subprocess."""
